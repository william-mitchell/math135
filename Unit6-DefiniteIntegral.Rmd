...
2. The car turns around at $t=6$. We have $v(t) > 0$ for $t < 6$ and $v(t) < 0$ for $t > 6$. 
...
$\displaystyle{\int_0^{30} K'(q) dq = \int_0^{30} \left( \frac{1}{100}q^2+\frac{1}{5}q+1 \right) dq }$
...
1. The sine curve is above the cosine curve. So the area is
$$\displaystyle{\int_{\pi/4}^{5\pi/4} ( \sin(x) - \cos(x)) dx}.$$

  
Hint: $\sin(x) = \cos(x)$ at $x=\pi/4$ and $x=5\pi/4$.
2. Our *RStudio* estimate is
