...
The define integral $\int_{a}^b f(x) dx$ is the (signed) area between the curve $y=f(x)$ and the $x$-axis. We approximate this area by partitioning the interval $[a,b]$ into smaller intervals of width $\triangle x$ and then summing the areas of $f(x) \triangle x$ of the corresponding rectangles.

Here is code that makes this approximation for the function $f(x) = x + sin(x^2)$ over the interval $[5,10]$. The code splits this interval into 100 subintervals. You can get a better approximation by increasing the denominator of `base=(b-a)/100` to a much larger nuumber, for example: `base=(b-a)/1000000`.
