...
$$
\frac{51.33 \mbox{ feet}}{1 \mbox{ sec}} \cdot \frac{12 \mbox{ inches}}{1 \mbox{ feet}} \cdot \frac{2.54 \mbox{ cm}}{1 \mbox{ sec}} = 1564.64 \frac{\mbox{cm}}{\mbox{sec}}
$$
...
V &=& k P^a g^b \mu^c \\
[V] &=& [P]^a [g]^b [\mu]^c \\
LT^{-1} &=& (L^{-1})^a (LT^{-2})^b (ML^{-3})^c \\
 L^1 T^{-1} M^0 &=& L^{a+b-3c} T^{-2b} M^{c}
\end{array}
$$
which means that $c=0$ and $b=1/2$ and $a=1/2$. So our final equation is
$$
V = k  \sqrt{Pg}
$$

### Fermi Estimation
  1. The population  of the earth is growing, but not very quickly.  So let's assume that the number of deaths is the same as the number of births. Life expectancy is 73 years, so about 1/73 of the population dies each year. So 1/73 of the population is  born each year, too
$$
...
$$
\frac{3 \mbox{ liters}}{1 \mbox{ day}}\frac{365 \mbox{ days}}{1 \mbox{ year}} \times (20 \mbox{ years}) = 21,900 \mbox{ liters}.
$$
Now we need to approximate the volume of a swimming pool. I know that olympic  swimming pools are $50$ meters long. The width smaller: let's guess  $30$ meters, and the depth is about 3 meters (not 1 meter). So the volume of a pool is $50 \times 30 \times 3 = 4500 m^3$. There are 1000 liters in a  cubic meter, so you've consumed about 22 cubic meters of liquids in your life. That's much smaller than one swimming pool. It would only have a depth of $22/(50*30)=0.015$  meters, which is 1.5 cm. 