...
8. $\sin(x) + 2\exp{x} + 7x + C$
9. $-\frac{1}{3} \cos(3x) - 2\sin(x) + C$
10. $-350 \exp{-0.02t} + 4\ln t + C$
...
1. Setting $f(x)=g(x)$ gives $5-x^2=x^2-3$ which means that $x^2=8$. So the curves  intersect at $x=\pm 2\sqrt{2}$.
2. On the interval $[-2\sqrt{2}, 2\sqrt{2}]$, function $f(x)$ is above function $g(x)$.
3. The area between these curves is
$$
\int_{-2\sqrt{2}}^{2\sqrt{2}} ( f(x) -g(x)) dx
=
\int_{-2\sqrt{2}}^{2\sqrt{2}} ( 8 -2x^2) dx
$$
...
$$
For very large $T$ the second term is basically 0. So we have 2000 zombies by the end of the outbreak.
